...
let arr= `[{"id":1,"first_name":"Abrahan","last_name":"Seabrocke","email":"aseabrocke1@ocn.ne.jp","gender":"Male","img_src":"https://robohash.org/autiuredistinctio.png?size=50x50&set=set1","class":3,"marks":27,"passing":true,"city":"Kampong Thom"},
...
let sortbyAZ = document.getElementById("sorta")   
let sortZA = document.getElementById("sortz")   
let sortMarks = document.getElementById("sortMarksInc")   
let showpas = document.getElementById("pasing")   
let sortclass = document.getElementById("class") 
let searchBtn = document.getElementById("searchBtn") 

sortMarks.addEventListener("click", sortMarksInc) ; 
showpas.addEventListener("click", showpass) ; 
sortclass.addEventListener("click", sortbyclass) ; 
    ...
    function loadData(data){
        ...
        lItem.innerHTML = `
    <td> ${item.id} </td>
    <td> <img src="${item.img}" height="40" width="40">
     ${item.first_name} 
     ${item.last_name} </td>
     <td> ${item.gender} </td>
     <td> ${item.class} </td>
     ...
     <td> ${item.city} </td>
    ` 
    product.append(lItem);
    
    })
}
loadData(data);

function sortMarksInc(a,b){
...
function sortbyclass(c,d){
    let increasingclass = data.sort((c,d)=>{
     if(c.class < d.class) return -1; 
     else if(c.class > d.class) return 1;
     else return 0;
    }) 
    loadData(increasingclass);
 }
 function showpass(){ 
    let showpassedstudent = data.filter((item)=>{
        return item.passing;
    })  
    loadData(showpassedstudent); 
}